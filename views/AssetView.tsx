...
const AssetView: React.FC<AssetViewProps> = ({ stats, setStats }) => {
  ...
  return (
    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-700">
      ...
      {showTierModal && (
        <div 
          className="fixed inset-0 z-[250] flex items-center justify-center px-4 bg-[#0b0e11]/95 backdrop-blur-2xl animate-in fade-in duration-300"
          onClick={(e) => {
            // 点击背景关闭弹窗
        ...
        >
          <div 
            className="bg-gradient-to-b from-[#1e2329] to-[#0b0e11] w-full max-w-sm rounded-[2rem] border border-white/10 shadow-[0_40px_100px_rgba(0,0,0,0.8)] animate-in zoom-in-95 slide-in-from-bottom-10 duration-500 overflow-hidden max-h-[85vh] flex flex-col"
          ...
          >
            ...
            <div className="relative p-5 pb-3 flex-shrink-0">
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="space-y-0.5">
                   <div className="flex items-center gap-2">
                      <Gem className="w-4 h-4 text-[#FCD535]" />
                      <h3 className="font-black uppercase tracking-[0.2em] text-xs text-white">{t('asset.vipPrivilege') || 'VIP 特权'}</h3>
                   </div>
                   <p className="text-[9px] text-[#848E9C] font-bold uppercase tracking-widest">{t('asset.holdToEarnRoadmap') || '持币生息路线图 v2.0'}</p>
                </div>
                <button 
              ...
              {/* Highlight Card */}
              <div className="bg-gradient-to-r from-[#FCD535]/10 to-transparent p-3 rounded-xl border border-[#FCD535]/20 flex items-center gap-3">
                <div className="w-10 h-10 bg-[#FCD535] rounded-xl flex items-center justify-center shadow-lg shadow-[#FCD535]/20">
                   <Zap className="w-5 h-5 text-[#0B0E11] fill-current" />
                </div>
                <div>
                   <p className="text-[10px] font-black text-white uppercase tracking-tight">{t('asset.holdToEarnMode') || '持币生息模式'}</p>
                   <p className="text-[8px] text-[#848E9C] font-bold uppercase tracking-tighter mt-0.5">{t('asset.walletHoldAutoInterest') || '钱包持币 • 自动计息 • 无需质押'}</p>
                </div>
              </div>
            </div>
            
            {/* Tiers List Section */}
            <div className="px-5 space-y-2.5 overflow-y-auto flex-1 pb-4 no-scrollbar">
              {VIP_TIERS.map((tier) => {
                const isActive = currentTier?.level === tier.level;
                return (
                <div 
                  key={tier.level} 
                  className={`relative p-4 rounded-xl border transition-all group ${isActive ? 'bg-white/5 border-[#FCD535]/50 shadow-[0_0_20px_rgba(252,213,53,0.05)]' : 'bg-[#1e2329]/40 border-white/5 opacity-60 grayscale-[0.5]'}`}
                >
                  {isActive && (
                    <div className="absolute -top-1.5 -right-1.5 bg-[#0ECB81] text-[#0B0E11] px-2 py-0.5 rounded-full text-[7px] font-black uppercase tracking-widest shadow-lg flex items-center gap-1">
                  ...
                  <div className="flex justify-between items-center mb-3">
                    <div className="flex items-center gap-3">
                       <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-base ${isActive ? 'bg-gradient-to-br from-[#FCD535] to-orange-400 text-[#0B0E11]' : 'bg-white/5 text-white/40'}`}>
                         V{tier.level}
                       </div>
                       <div>
                         ...
                         <p className="text-[8px] text-[#848E9C] font-bold uppercase tracking-[0.1em]">{t('asset.protocolNode') || '协议节点'}</p>
                       </div>
                    </div>
                    <div className="text-right">
                       <p className={`text-xl font-black mono leading-none tracking-tighter ${currentTier?.level === tier.level ? 'text-[#FCD535]' : 'text-white/20'}`}>{tier.dailyRate}%</p>
                       <p className="text-[7px] text-[#848E9C] font-bold uppercase tracking-widest mt-0.5">{t('asset.dailyRate') || '日利率'}</p>
                    </div>
                  </div>
            ...
            {/* Footer Section */}
            <div className="p-5 bg-black/40 border-t border-white/5 flex-shrink-0">
               <button 
                 onClick={() => setShowTierModal(false)}
                 className="w-full bg-[#1e2329] border border-white/10 hover:bg-white/5 text-white font-black py-4 rounded-xl text-[10px] uppercase tracking-[0.2em] transition-all active:scale-95 flex items-center justify-center gap-2"
               >
                 {t('asset.acknowledgePrivileges') || '确认特权'}
                 <ChevronRight className="w-3.5 h-3.5" />
               </button>
               <p className="text-[7px] text-center text-[#848E9C] font-bold uppercase tracking-widest mt-3 opacity-50">{t('asset.protocolLayer') || 'Rabbit AI 去中心化协议层 2'}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {showWithdrawModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center px-6 bg-black/90 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-[#1e2329] w-full max-w-sm rounded-[2rem] border border-white/10 shadow-[0_0_50px_rgba(0,0,0,1)] animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-white/5 flex justify-between items-center">
              <h3 className="font-black uppercase tracking-widest text-sm">Withdrawal Node</h3>
            ...
            <div className="p-7 space-y-6">
              ...
              <div className="bg-black/20 p-5 rounded-2xl border border-white/5 space-y-3">
                 ...
                 <div className="flex justify-between text-[10px] font-bold uppercase pt-2 border-t border-white/5">
              ...
              <button 
                ...
                className="w-full bg-[#FCD535] text-[#0B0E11] font-black py-5 rounded-2xl disabled:opacity-20 disabled:cursor-not-allowed text-sm uppercase tracking-[0.2em] shadow-lg shadow-[#FCD535]/10 active:scale-95 transition-all"
              >
