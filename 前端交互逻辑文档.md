...
3. **能量不足时的交互限制**：
   - 输入框可用（不再禁用）
   - MAX 按钮可用
   - 提交按钮根据能量是否足够支付本次提现来禁用
   - 显示能量不足提示信息（仅针对当前提现金额）
4. 用户可以：
...
- **能量门槛**：无（已取消），只需满足 1 USDT = 10 能量的关系
- **能量消耗**：1 USDT = 10 能量
- **能量信息卡片**：在提现 Modal 中始终显示，包含当前能量、所需能量
...
**能量获取方式**：
- 每日空投领取：+1 能量
- 邀请好友成功（首次）：+2 能量（邀请奖励）+ 1 能量（管道收益）= 3 能量
- 下级领取空投（后续）：+1 能量（管道收益）

**能量消耗规则**：
- 提现收益：1 USDT = 10 能量
- 最低提现门槛：无（已取消，只需满足 1 USDT = 10 能量的关系）
